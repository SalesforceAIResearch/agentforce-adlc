...
- Skill `metadata.version` bumped for the voice + review changes: `agentforce-generate` 0.9→0.11, `agentforce-test` 0.6→0.8, `agentforce-observe` 0.6→0.8. Plugin version bumped 0.9.0→0.11.0 in both `plugin.json` and `marketplace.json`. ([#39](https://github.com/SalesforceAIResearch/agentforce-adlc/pull/39), voice latency/authoring additions)
