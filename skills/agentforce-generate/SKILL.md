...
1. **Ask the user approval to download schema and data from the org** - Ask the user: *"Do you want me to extract use-cases and map them to objects in your org to generate testing queries for agent creation? This will also download schema and data from your org."*. Provide options as either yes or no. Let the user select one  **Do not proceed** without approval. If the user approves, run use-case extraction using [Extract Use Cases](references/extract-usecases.md), then run schema selection and mapping for all relevant objects using [Schema Selection](references/schema-selection.md). Save the generated schema and use-case mapping JSON to `artifacts/schema-selection.json` before presenting results. Present all three - the extracted use cases, schema, and its mapping to the user. Now, download all the data for the objects (core + parent + supporting + any other objects) in the schema using [Schema Data Download](references/schema-data-download.md) and save it in `artifacts` directory. After data download completes, generate grounded testing queries using [Test Query Generation](references/test-query-generation.md) and save outputs under `artifacts/`.
