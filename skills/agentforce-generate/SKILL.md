...
4. **Execute the `sf agent mcp` command** — Always include `--json` (Rule 1). Parse the structured response; extract the server `id` after create for subsequent operations.
5. **Interactive whitelisting (for asset activation)** — When whitelisting tools, display each asset's full metadata (description, input/output schemas, annotations, current status) and wait for explicit yes/no/skip approval per tool before building the allowlist. `sf agent mcp asset replace` is a FULL replacement — include the complete desired state.
