...
1. **Ask the user approval to download schema and data from the org** - Ask this Step 1 approval question **alone** (do not combine it with any other question or prompt): *"Do you want me to extract use-cases and map them to objects in your org to generate testing queries for agent creation? This will also download schema and data from your org."*. Provide options as either yes or no. Let the user select one. **Do not proceed** to Step 2 or ask additional questions until the user responds to this Step 1 approval prompt. If the user approves, run use-case extraction using [Extract Use Cases](references/extract-usecases.md), then run schema selection and mapping for all relevant objects using [Schema Selection](references/schema-selection.md). Save the generated schema and use-case mapping JSON to `artifacts/schema-selection.json` before presenting results. Present all three - the extracted use cases, schema, and its mapping to the user. Now, download the data for the objects (core + parent + supporting + any other objects) in the schema using [Schema Data Download](references/schema-data-download.md) and save it in `artifacts` directory. After data download completes, generate grounded testing queries using [Test Query Generation](references/test-query-generation.md) and save outputs under `artifacts/`.
2. **Design** — Read [Design & Agent Spec](references/agent-design-and-spec-creation.md) to draft an Agent Spec. If Step 1 produced test queries under `artifacts/` (from [Test Query Generation](references/test-query-generation.md)), use those queries as design input: structure subagents, actions, instructions, and data access so the agent can correctly answer **all** generated testing queries (including their ground-truth intents), and also handle related user asks **beyond** that exact set — adjacent phrasings, follow-ups, and in-scope variations not literally listed in the test pack. Treat the test queries as a required coverage baseline, not the full ceiling of agent capability. Default all new actions to `NEEDS STUB` placeholders during planning. Ask the user which implementation path they want before implementation work:
...
3. **STOP for user approval of Agent Spec.** Present to user (including the Knowledge Grounding section if present). Ask for approval or feedback. **Do not proceed** without approval. Once approved, proceed without stopping unless a step fails.
4. **Validate environment prerequisites** — Read [Design & Agent Spec](references/agent-design-and-spec-creation.md), Section 3 (Environment Prerequisites). Based on agent type from design, validate org environment:
...
8. **Generate action implementations (explicit user-requested path only)** — Only run this step if the user explicitly asked to generate new implementations (Path C in Step 1). For each action marked NEEDS STUB:
...
10. **Publish (explicit release step)** — Only after the user confirms they are ready to commit this draft to metadata. Publish validates metadata structure, not agent behavior. Every publish creates permanent version number.
...
11. **Activate (explicit release step)** — Makes new version available to users after publish.
...
19. [Extract Use Cases](references/extract-usecases.md) — classify prompt specificity and produce use case/sub-task JSON from prompt context
20. [Schema Selection](references/schema-selection.md) — fetch org objects/fields via SF CLI and map use cases to schema for test-query grounding
21. [Schema Data Download](references/schema-data-download.md) - download data for objects in schema via SF CLI and save it to local
22. [Test Query Generation](references/test-query-generation.md) — schema subgraphs, start-record traversal, template grounding, paraphrase, and user_context metadata
