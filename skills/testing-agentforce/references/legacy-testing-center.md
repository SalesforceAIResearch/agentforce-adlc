# Legacy Testing Center (`AiEvaluationDefinition`) — Reference Only

> ⚠️ **DEPRECATION NOTICE**
>
> `/testing-agentforce` does **not** author legacy `AiEvaluationDefinition` test specs as of v0.7.0. NGT (`AiTestingDefinition`) is the only mode the skill supports — see `references/batch-testing.md`.
>
> **If you open an existing `<Name>.aiEvaluationDefinition-meta.xml`:** the skill will not author or extend it. Use this doc as a schema reference for hand-edits, or pin `agentforce-adlc@0.6.x` for active legacy authoring support.
>
> **Migrating to NGT** is a manual rewrite, not an automatic conversion. Field-level mapping:
> - `expectedTopic` → `topic_sequence_match`
> - `expectedActions` → `action_sequence_match`
> - `expectedOutcome` (LLM-as-judge) → `bot_response_rating`
>
> Legacy authoring will not get new features. Bug fixes are accepted on a best-effort basis.

---

# Legacy Testing Center Batch Testing — Schema Reference

Testing Center is Salesforce's older test infrastructure for Agentforce agents. Tests are deployed as `AiEvaluationDefinition` metadata to the org and can be run via CLI or Setup UI. This document describes the schema and CLI commands for users maintaining pre-existing legacy suites.

## Phase 1: Test Spec YAML

The legacy Testing Center uses a flat YAML format, one entry per test case:
...
- **Always add `expectedOutcome`** -- it is the most reliable assertion type and works even when topic/action assertions can't capture nuanced behavior
...
`sf agent test create` (without `--test-runner`) takes the YAML spec, converts it to `AiEvaluationDefinition` metadata XML, and deploys it to the org. The XML is written to `force-app/main/default/aiEvaluationDefinitions/` as part of the SFDX project.
...
Derive a legacy Testing Center spec from the `.agent` file:
