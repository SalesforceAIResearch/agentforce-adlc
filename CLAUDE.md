...
- `/agentforce-test` (Mode C — OWASP LLM Top 10 security testing) is part of the test flow, not a separate skill: it generates deployable Testing Center security test cases (C1) and/or runs live adversarial probing with an A–F grade (C2). Generating security test cases requires **explicit user confirmation**. Cases are generated **from the agent's own `.agent` script** — its actions, `available when` gates, injection sinks, and inferred business domain — so always pass `--agent-file`; a generic-library-only run covers materially less and must say so in the report.
